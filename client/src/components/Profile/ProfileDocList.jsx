...
function ProfileDocList() {
  ...
  useEffect(() => {
    ...
    const fetchApi = async () => {
      ...
      try {
        ...
        setDocument({ documents: filteredDocs, pages: result.pages }); // vẫn giữ total pages
      } catch (err) {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      setLoading(true);
      try {
        const res = await getPosts(); // res là object: { message, posts: [...] }
      ...
      <div className="mb-4">
        <div className="bg-white rounded-lg shadow border-b border-gray-200">
          <nav className="flex -mb-px">
            <button
              className={`py-3 px-6 text-sm font-medium border-b-2 transition ${activeTab === "docs"
            ...
            <button
              className={`py-3 px-6 text-sm font-medium border-b-2 transition ${activeTab === "posts"
                ? "border-blue-600 text-blue-600"
                : "border-transparent text-gray-500 hover:text-blue-600 hover:border-blue-300"
                }`}
